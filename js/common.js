...
function getWeatherEmojiHtml(code){
    if(code===0 && isNight()){
        if(emojiSet==='fluent') return '<img src="emoji/Crescent moon.svg" style="width:100%;height:100%;" onerror="this.style.opacity=0">';
        else return '🌙';
    }
    if(emojiSet==='fluent'){
        const f=FLUENT_SVG_MAP[code]||'Cloud.svg';
        return `<img src="emoji/${f}" style="width:100%;height:100%;" onerror="this.style.opacity=0">`;
    }
