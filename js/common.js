...
const FLUENT_SVG_MAP = {
    ...
    51:'Sun behind rain cloud.svg',
    53:'Sun behind rain cloud.svg',
    55:'Sun behind rain cloud.svg',
    61:'Cloud with rain.svg',
    63:'Cloud with rain.svg', 
    65:'Cloud with rain.svg', 
    71:'Cloud with snow.svg',
    73:'Cloud with snow.svg', 
    75:'Cloud with snow.svg', 
    80:'Cloud with rain.svg',
    81:'Cloud with rain.svg', 
    82:'Cloud with rain.svg', 
    85:'Cloud with snow.svg',
    86:'Cloud with snow.svg', 
    95:'Cloud with lightning.svg',
    96:'Cloud with lightning and rain.svg', 
    99:'Cloud with lightning and rain.svg'
};

const FLUENT_NIGHT_MAP = {
    0:'Crescent moon.svg',
    1:'Crescent moon.svg',
    2:'Crescent moon.svg',
    3:'Cloud.svg',
    45:'Fog.svg',
    48:'Fog.svg',
    51:'Cloud with rain.svg',
    53:'Cloud with rain.svg',
    55:'Cloud with rain.svg',
    61:'Cloud with rain.svg',
    63:'Cloud with rain.svg',
    65:'Cloud with rain.svg',
    71:'Cloud with snow.svg',
...
function isNight(){ 
    ...
    const n = new Date(), s = new Date(sunsetTime); 
    s.setMinutes(s.getMinutes()+30); 
    return n > s; 
}

function isTimeNight(timeString) {
    if (!cachedWeatherData?.daily?.sunrise?.[0] || !cachedWeatherData?.daily?.sunset?.[0]) {
        const hour = new Date(timeString).getHours();
        return hour < 6 || hour >= 22;
    }
    const time = new Date(timeString);
    const sunrise = new Date(cachedWeatherData.daily.sunrise[0]);
    const sunset = new Date(cachedWeatherData.daily.sunset[0]);
...
function getWeatherEmojiForTime(code, timeString) {
    const isNightTime = isTimeNight(timeString);
    
    if (emojiSet === 'fluent') {
        if (isNightTime) {
            const f = FLUENT_NIGHT_MAP[code] || FLUENT_SVG_MAP[code] || 'Cloud.svg';
...
function updateBackground(code){
    ...
    if(!bg) return;
    const cat = ['clear','partly_cloudy','cloudy','fog','rain','snow','thunderstorm','default'][
...
async function fetchWeatherData(lat, lon, forceRefresh = false){
    ...
    for(const u of API_URLS){
        try{
            ...
            if(r.ok){
                const d = await r.json(); 
                if(d.daily?.sunset?.[0]) sunsetTime = d.daily.sunset[0];
                cachedWeatherData = d;
...
function updateMenuUI(){
    document.querySelectorAll('[data-theme]').forEach(i => 
        i.classList.toggle('active', i.dataset.theme === currentTheme)
    );
    document.querySelectorAll('[data-emoji]').forEach(i => 
        i.classList.toggle('active', i.dataset.emoji === emojiSet)
    );
}
